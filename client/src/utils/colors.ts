...
type Bg = {
  [Property in keyof typeof colors]: { backgroundColor: string }
}

export const backgroundColors: Bg = Object.entries(colors).reduce(
  (prev, cur) => ({
    ...prev,
    [cur[0]]: {
...
export const progressColor = (percentage: number, isWorkingDay: boolean) => {
  if (isWorkingDay) {
    switch (true) {
      case percentage === 100:
        return COMPLETED
